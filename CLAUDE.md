...
- `npm run build` output is already minified — CSS/JS via Vite's default `esbuild` minifier, HTML via `vite-plugin-html`'s built-in `html-minifier-terser` step (`minify: true`, set explicitly in `vite.config.js` even though it's also the plugin's default). Comments (including the `TODO`s scattered through `index.html`) are stripped from the production build, so don't rely on view-source in prod to find them.
...
### Mobile nav drawer
The horizontal `<nav>` bar is `hidden md:block` — below `md` it's replaced by `#mobile-menu`, a right-side off-canvas panel (`translate-x-full` ⇄ untransformed, `transition-transform`), opened by a hamburger button placed right after the company name in `<header>` (`#menu-toggle`, `md:hidden`) and closed by its own `✕` button, an overlay click, Escape, a nav-link click, or resizing past `md`. Because `--nav-h` is measured from the (now `display:none` on mobile) `<nav>`, it naturally reads `0` on mobile, so the `#hero` height calc above doesn't need a mobile-specific branch — it just works out. Logic lives in `src/script/mobile-menu.js`; both nav lists (desktop `<nav>` and the drawer's) are separate, hand-duplicated markup — if you add/remove a page section, update both.
...
### Fonts
Self-hosted Montserrat, one static WOFF2 per weight actually used on the page — **not** the variable font anymore. `src/styles/fonts.css` has five `@font-face` blocks (400/500/600/700/800, all `font-style: normal`), each pointing at `src/styles/Montserrat/Montserrat-<Weight>.woff2`. No italic anywhere in `index.html`, so there's no italic `@font-face` at all.

If you add a new `font-*` Tailwind class (a weight not in that list) or start using `italic`, you must add a matching `@font-face` block yourself — the browser will otherwise fake it (synthetic bold/oblique) instead of loading a real face. Source for any weight/style, including italics, is the full Google Fonts download kept at `src/styles/_fonts/Montserrat/static/` (TTF, not wired into the build) — convert whichever file you need to WOFF2 (e.g. via `fontTools`: `TTFont(...); font.flavor = 'woff2'; font.save(...)`) and drop it into `src/styles/Montserrat/`. Don't re-add the two `*-VariableFont_wght.ttf` files as a shortcut — that's what this cleanup removed, since it downloads the entire 100–900 design space (~690 KB) to render 5 discrete weights that total ~490 KB as separate static WOFF2s.
...
- `lead-form.js` — handles `#lead-form` submit: client-side required-phone check, disables the submit button and shows an inline status message while in flight, `fetch('/api/lead', …)`. **`/api/lead` does not exist** — there is no backend yet.

  **Planned**: a Node.js server (its own project, not this repo) that serves this site's `dist/` build as static files *and* exposes `POST /api/lead` on the same origin, forwarding submissions to Telegram/WhatsApp/MAX. Same-origin means `fetch('/api/lead', …)` needs no change and there's no CORS to configure — that complexity only existed under a split-hosting plan that's since been dropped in favor of one Node server doing both jobs.

  This is a **separate future deployment target from GitHub Pages, not a replacement yet** — GitHub Pages (and the `base: '/nikolai/'` in `vite.config.js` it requires) stays as the current deploy target until that Node hosting actually exists. When it's time to switch: change `base: '/nikolai/'` to `base: '/'` in `vite.config.js` (Node serving from a domain root has no `/nikolai/` subpath to account for), and decide whether `.github/workflows/main.yml` / GitHub Pages still serves any purpose (e.g. as a preview) or should be retired.
...
- `mobile-menu.js` — drives `#mobile-menu`, a right-side off-canvas drawer (see "Mobile nav drawer" below): open/close via `#menu-toggle`/`#menu-close`/overlay click/Escape, auto-closes on link click or on resize past the `md` breakpoint (so it can't get stuck open, scroll-locked, behind the desktop nav).
...
**`<meta name="robots" content="noindex, nofollow">` is currently set in `<head>`** — the site is a work-in-progress test deployment and must not be indexed by Yandex/Google yet. This is intentionally *not* done via `robots.txt` (a `Disallow` there would stop crawlers from ever reading the page and seeing the `noindex` tag, which can paradoxically leave the bare URL indexed if it's linked from anywhere). **Remove this meta tag when the site is actually ready to launch** — it's the one thing in this repo that, if forgotten, silently kills all SEO effort.

### Deployment (GitHub Pages)
`.github/workflows/main.yml` deploys `dist/` via the official GitHub Pages Actions flow (`actions/upload-pages-artifact` + `actions/deploy-pages`, two jobs: `build` → `deploy`) — this matches the repo's Settings → Pages → Source being set to **"GitHub Actions"**; nothing else to configure there. It needs `permissions: pages: write` + `id-token: write` (not `contents: write` — this method doesn't push to any branch, so there's no `build`/`gh-pages` branch involved at all).

**Trigger is `workflow_dispatch` only — it does not run on push.** Deploying is a deliberate action (repo's Actions tab → this workflow → "Run workflow"), not something that happens automatically from a commit. This is intentional while the site is still a `noindex` test build (see below) — re-add a `push: branches: [main]` trigger once it's actually meant to auto-publish on every merge.

The repo is `github.com/Ilya006/nikolai`, so Pages serves it as a **project page** at `/nikolai/`, not at the domain root. Because of that, `vite.config.js` sets `base: '/nikolai/'` **only when `command === 'build'`** (dev stays at `/`, so `npm run dev` is unaffected) — if the repo is ever renamed, or the site moves to a custom domain / user root page (`Ilya006.github.io`), that hardcoded `/nikolai/` needs to change too, or every asset URL in the production build breaks.
